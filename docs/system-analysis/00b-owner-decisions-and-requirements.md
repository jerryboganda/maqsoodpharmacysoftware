...
| **D18** | 2026‑08‑02 | DRAP / controlled-drug regulatory obligations (U-062)? | **YES — real obligations exist. Research them fully and build compliant record-keeping, but it must be "super easy" to use.** Initial research done — see R7. **Wave 8 (2026‑08‑05) built the generic, no-regrets record-keeping infrastructure this decision calls for**: a per-branch DRAP licence number + expiry (with an automatic renewal-reminder alert), an optional one-field dispensing note captured inline at the point of sale for any item flagged controlled, and a Controlled Drug Register report. None of this invents the actual legal register, retention period, or countersignature rule DRAP may require — those remain the open part below. Full compliance scope still needs a licensed pharmacist/regulatory consultant to finalise; this analysis does not invent regulated-domain requirements from web research alone. | 🟡 Record-keeping infrastructure built; finalisation still pending professional sign-off |
...
**Wave 8 (2026‑08‑05) — what was actually built against this design commitment**, deliberately scoped to the parts above that are generic and professionally-uncontroversial, never guessing at the still-open items:
- `branch.drugSaleLicenceNo` / `branch.drugLicenceExpiryDate` (mirrors D17's ntnNo/strnNo pattern exactly) plus a first-ever admin surface to edit a branch at all (`GET`/`PATCH /settings/branches`) — D17 already called this kind of field "admin-editable settings", but no endpoint existed anywhere in this codebase to edit one until now.
- A 5th notification condition (`licence_expiring`, reusing the existing `scanForAlerts`/`reconcileKind` machinery) that warns the owner when a branch's licence is within a defensible-default 60-day window — the exact number is this task's own pick, not a DRAP-mandated figure, same convention `PURCHASE_ORDER_STALE_DAYS` already established for an identical gap.
- `sale_invoice_line.dispensingNote` — the literal "one extra, clearly-labelled field at the point it's already being rung up" this section calls for, wired through POS checkout, sale posting, and a new **Controlled Drug Register** report (`GET /reports`'s 9th entry). One free-text field, not a rigid structured buyer/CNIC/prescription-number scheme — the exact structure remains one of the still-open items below.
- All of it optional and unenforced: nothing here blocks a sale, requires a value, or claims legal compliance. It exists so the record CAN be kept the moment a pharmacist/regulatory consultant confirms what's actually required.
- Live-verified end to end in a real browser against real MySQL (not just unit-tested) — see `pharmacy-rebuild-full-scope-audit.md` (session memory) for the verification trail.
